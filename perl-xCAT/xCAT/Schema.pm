...
'adddhcpstatements' => 'Custom dhcp statements for administrator use (not implemneted yet)',
    ...
    chain => {
        ...
        descriptions => {
            ...
            chain => 'A comma-delimited chain of actions to be performed automatically when this node is discovered for the first time.  (xCAT and the DHCP server do not recognize the MAC address of the node when xCAT initializes the discovery process.)  The last step in this process is to run the operations listed in the chain attribute, one by one.  Valid values:  boot, runcmd=<cmd>, runimage=<URL>, shell, standby. For example, to have the genesis kernel pause to the shell, use chain=shell. The legacy Genesis image runs runimage. The OpenEmbedded Genesis image refuses runimage and reports UNSAFE_LEGACY_ACTION; build a signed system extension with xCAT-genesis-builder/oe/export-extension.',
...
',
    ...
    osimage => {
        cols => [qw(imagename groups profile imagetype description provmethod rootfstype nfsrootopts osdistroname osupdatename cfmdir osname osvers osarch synclists postscripts postbootscripts serverrole isdeletable kitcomponents environvar comments disable)],
        keys       => [qw(imagename)],
        tablespace => 'XCATTBS32K',
        ...
        descriptions => {
            ...
            nfsrootopts => 'A comma-separated list of additional NFS mount options for the root filesystem of a dracut-based statelite image. The root filesystem is always mounted read-only; rw and defaults are not allowed.',
...
@{ $defspec{osimage}->{'attrs'} } = (
    ...
    { attr_name => 'nfsrootopts',
        only_if         => 'imagetype=linux',
        tabentry        => 'osimage.nfsrootopts',
        access_tabentry => 'osimage.imagename=attr:imagename',
    },
    { attr_name => 'profile',
