        ...
        for ($i = 0 ; $i < scalar(@currnamed) ; $i++) {
            ...
            if ($line =~ /^options +\{/) {
                ...
                if ($ctx->{privkey}) {
                    ...
                    # named matches a TSIG key by name and by algorithm, and refuses to
                    # load a key statement with no algorithm clause. Repair a stanza that
                    # names none, and replace one that names another algorithm than the
                    # site table selects.
                    if (!$algorithmnow
                        || ($omapi_settings->{algorithm_explicit}
                            && lc($algorithmnow) ne $omapi_settings->{algorithm}))
                    {
