...
B<rspconfig> I<noderange> B<alert>

B<rspconfig> I<noderange> B<alert>={I<ip:port>|B<off>|B<disable>}

B<rspconfig> I<noderange> B<snmpdest>

B<rspconfig> I<noderange> B<snmpdest>=I<snmpmanager-IP[:port]> [B<-d>|B<--delete>]

B<rspconfig> I<noderange> B<userls>

B<rspconfig> I<noderange> B<useradd>=I<user> B<password>=I<password> B<privilege>={B<priv-admin>|B<priv-operator>|B<priv-user>|B<priv-callback>} [B<groups>={B<ipmi,redfish,ssh,web>}] [B<enabled>={B<0>|B<1>}]

B<rspconfig> I<noderange> B<userch>=I<user> [B<password>=I<password>] [B<privilege>={B<priv-admin>|B<priv-operator>|B<priv-user>|B<priv-callback>}] [B<groups>={B<ipmi,redfish,ssh,web>}] [B<enabled>={B<0>|B<1>}]

B<rspconfig> I<noderange> B<userrm>=I<user>
...
=item B<alert> | B<alert>={I<ip:port>|B<off>|B<disable>}

For OpenBMC nodes, show or set the remote syslog destination. Specifying B<off> or B<disable> disables remote logging.
...
=item B<snmpdest> | B<snmpdest>=I<snmpmanager-IP[:port]> [B<-d>|B<--delete>]
...
=item B<useradd>=I<user> B<password>=I<password> B<privilege>={B<priv-admin>|B<priv-operator>|B<priv-user>|B<priv-callback>} [B<groups>={B<ipmi,redfish,ssh,web>}] [B<enabled>={B<0>|B<1>}]

For OpenBMC nodes, create a local BMC user. If B<groups> is omitted, all supported OpenBMC groups are used. If B<enabled> is omitted, the user is enabled.

=item B<userch>=I<user> [B<password>=I<password>] [B<privilege>={B<priv-admin>|B<priv-operator>|B<priv-user>|B<priv-callback>}] [B<groups>={B<ipmi,redfish,ssh,web>}] [B<enabled>={B<0>|B<1>}]

For OpenBMC nodes, update the password, privilege, groups, or enabled state for a local BMC user.

=item B<userrm>=I<user>
